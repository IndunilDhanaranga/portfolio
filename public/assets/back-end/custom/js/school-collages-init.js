...
function addSchool(school,from,to) {
    ...
    let html = '<div class="row mt-2" id="div_' + index + '">' +
        ...
        '<input type="text" class="form-control" placeholder="School" value="' + (typeof school !== 'undefined' ? school : '') + '" id="school_' + index + '" name="school[]">' +
        '</div>' +
        '<div class="col-md-3 col-6">' +
        '<label for="from_' + index + '">From</label>' +
        '<input type="text" class="form-control" placeholder="From" value="' + (typeof from !== 'undefined' ? from : '') + '" name="from[]">' +
        '</div>' +
        '<div class="col-md-3 col-6">' +
        '<label for="to_' + index + '">To</label>' +
        '<input type="text" class="form-control" placeholder="To" value="' + (typeof to !== 'undefined' ? to : '') + '" name="to[]">' +
        '</div>' +
        '</div>';
